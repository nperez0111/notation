...
function SidebarDropStrip({
  ...
  onDrop: (documentId: number, insertAtIndex?: number) => void | Promise<void>;
  insertAtIndex?: number;
    ...
    return dropTargetForElements({
      ...
      onDrop: ({ source }) => {
        ...
        if (data.type === "sidebar-doc" && typeof data.documentId === "number") {
          void onDrop(data.documentId, insertAtIndex);
        }
        setActiveDrop(null);
      },
    });
...
function DocumentTreeItem({
  ...
  const handleReparent = (
    ...
    void onReparentDocument?.(documentId, targetCollectionId, targetParentId, insertAtIndex);
  };
  ...
  return (
    <>
      <SidebarDropStrip
        ...
        onDrop={(id, insertAtIndex) =>
          handleReparent(id, collectionId, doc.parentId, insertAtIndex)
        }
        insertAtIndex={siblingIndex >= 0 ? siblingIndex : undefined}
      />
      <div ref={rowRef} className="list-row cursor-grab active:cursor-grabbing" role="listitem">
        <DocumentListItem
          document={doc}
          ...
          depth={depth}
          onCreateChild={
            collectionId != null ? () => onCreateDocument(collectionId, doc.id) : undefined
          }
...
export function CollectionSection({
  ...
  const submitRename = () => {
    ...
    if (trimmed && trimmed !== collection.name && onRenameCollection) {
      void onRenameCollection(collection.id, trimmed);
    }
          ...
          <div className="mt-0 flex flex-col gap-0 px-2" role="list">
            {onReparentDocument && (
              <SidebarDropStrip
                ...
                onDrop={(id, insertAtIndex) =>
                  onReparentDocument(id, collection.id, null, insertAtIndex)
                }
                insertAtIndex={0}
              />
            )}
