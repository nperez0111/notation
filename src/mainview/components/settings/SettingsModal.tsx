...
export function SettingsModal({
	...
	useEffect(() => {
		if (isOpen) {
			setPendingPath(null);
			rpc.getSettings({}).then(setSettings);
		}
	}, [isOpen, rpc]);
			...
			<div className="flex flex-col gap-6 p-1">
							...
							{pendingPath ? (
								<div className="flex flex-col gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-3">
									<p className="text-sm text-[var(--color-text)]">
										...
										{settings.documentCount !== 1 ? "s" : ""}. How do you want
				...
				<section className="flex flex-col gap-2">
					...
					<Select
						...
						value={[{ id: theme, label: THEME_OPTIONS.find((o) => o.id === theme)?.label ?? theme }]}
						onChange={({ value }) => {
